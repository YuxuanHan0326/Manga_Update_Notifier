...
- CopyManga 更新检测增加风控回退：当章节 API 返回 `code=210` 等受限响应时，会自动回退到网页元信息最新话进行检测，避免“手动检查一直 0 更新”
- CopyManga 请求特征已对齐官方客户端（`COPY/3.0.0` + `version/platform/webp/region`）并启用短超时有界重试，降低临时网络抖动导致的误失败
- 兼容历史异常状态：若旧订阅曾因异常导致 `last_seen_update_id` 丢失，但保留了较旧的 Last Seen 标题，修复后首次检查会补发一次最新更新事件
...
- 订阅封面加载策略：优先通过应用内封面代理加载，降低外链防盗链/跨域策略导致的渲染失败
  ...
  - CopyManga 部分封面 CDN 即使返回图片字节也可能标记为 `binary/octet-stream`，代理已兼容并按图片类型回传，避免出现“有封面链接但前端不显示”
...
- Trivy 门禁策略补充：
  - Trivy 使用 `--ignore-unfixed`：仍然阻断 `HIGH/CRITICAL`，但仅针对“已有修复版本”的漏洞。
  - 对上游“暂无修复版本”的基础镜像漏洞（例如 Debian 临时无补丁窗口）不做硬阻断，避免流水线长期不可通过。
- 常见失败解释：
