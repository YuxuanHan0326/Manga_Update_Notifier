...
- `app.scheduler`: APScheduler manager with two cron jobs:
  ...
  - `daily_summary`
- `app.adapters`:
  - `base.py`: adapter contract (`search`, `list_updates`, `healthcheck`)
  - `registry.py`: adapter registry
  - `copymanga.py`: Phase 1 source adapter
- `app.services`:
  ...
  - `settings.py`: runtime config persistence/merge
...
- `system_settings`: persisted runtime settings (cron/timezone/channels).
