...
|---|---|---|---|---|
| T-067 | Align CopyManga adapter request fingerprint with reference client and add robustness tests | FR-002, FR-004, FR-005, NFR-001, NFR-002 | 2026-03-19 | Centralized strict CopyManga API header contract, aligned chapter API `platform=1`, added bounded short-timeout retry wrapper, and added unit regressions for list-updates header consistency + retry + healthcheck header path |
| T-066 | Debug and fix CopyManga update-detection miss (manual run shows no update despite remote new chapter) | FR-002, FR-005, FR-006, NFR-001 | 2026-03-19 | Root cause: CopyManga chapter API returned risk-control payload (`code=210`) so chapter list stayed empty and `last_seen_update_id` never seeded; fix: add chapter-API fallback to web-meta latest chapter and numeric chapter-index sorting; added unit regression tests + checker baseline-change test; full tests passed and Docker runtime verification completed |
